...
def show_results(results: dict) -> None:
    """Show the metadata in a more readable format

    Args:
        results (dict): The results generated by reading from the CSV file
    """
...
def store_result_in_csv(
    ...
    """Store a web page download result in a given csvfile,
...
def get_elements(soup: bs4.BeautifulSoup, element: str) -> bs4.element.ResultSet:
    """Get all the HTML elements of a certain type from the web page.

    Args:
        soup (bs4.BeautifulSoup): A beautiful soup object
        element (str): The type of HTML element to find

    Returns:
        bs4.element.ResultSet: An object containing an array of
        beautiful soup objects representing the HTML elements found
    """
...
def download_assets(
    ...
    """Download additional assets, such as images, from the web page

    Args:
        session (requests.sessions.Session): A session object from a previous request
        soup (bs4.BeautifulSoup): A beautiful soup object
        ref_map (dict[str, str]): A map of the HTML element's tags to its references
        (e.g. <a> and "href")
        url (str): The URL being fetched
        path (str): The path where the assets should be saved
    """
...
def save_html(html: str, path: str) -> None:
    ...
    Args:
        html (str): A string containing the HTML content of a web page
        path (str): The path where the HTML file should be saved
    """
    save_path = Path(path)
...
def fetch(url: str, csvfile: str) -> None:
    """Fetch the web page and save it to disk

    Args:
        url (str): The URL being fetched
        csvfile (str): A CSV file to hold metadata results

    Raises:
        typer.Abort: An error check for valid URLs
    """
...
def main(urls: list[str], metadata: bool = False):
    ...
    Args:
        urls (list[str]): A list of URLs from the command line
        metadata (bool, optional): A flag to set whether to
